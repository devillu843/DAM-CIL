...
from torch.nn import ModuleList, Linear, ReLU, Conv2d, MaxPool2d, Dropout
...
def gs_cal(t, x, y, criterion, model, sbatch=20):
    ...
    for i in range(0, x.size(0), sbatch):
        b = torch.LongTensor(np.arange(i, np.min([i + sbatch,
                                                  x.size(0)]))).cuda()
        images = x[b]
        target = y[b]
        ...
        for name, param in model.named_parameters():
            if len(param.size()
                   ) <= 1 or 'last' in name or 'downsample' in name:
                continue
            name = name.split('.')[:-1]
            name = '.'.join(name)
    ...
    for module_ in model.net:
        if isinstance(module_, ModuleList):
            for linear_ in module_:
                linear_.clean()
        if isinstance(module_, ReLU) or isinstance(
                module_, Linear) or isinstance(module_, Conv2d) or isinstance(
                    module_, MaxPool2d) or isinstance(module_, Dropout):
            module_.clean()
...
def compute_mean_std_dataset(dataset):
    ...
    mean_expanded = mean.view(mean.size(0), mean.size(1), 1,
                              1).expand_as(image)
    for image, _ in loader:
...
def logit_adjustment(train_loader, tro=1):
