...
class SplitCosineLinear(nn.Module):
    ...
    def __init__(self,
        ...
        self.nb_proxy = nb_proxy
        self.fc1 = CosineLinear(in_features, out_features1, nb_proxy, False,
                                False)
        self.fc2 = CosineLinear(in_features, out_features2, nb_proxy, False,
                                False)
        if sigma:
            self.sigma = nn.Parameter(torch.Tensor(1))
            self.sigma.data.fill_(1)
    ...
    def forward(self, x):
        ...
        out = torch.cat((out1['logits'], out2['logits']),
                        dim=1)  # concatenate along the channel
